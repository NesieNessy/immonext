...
import type { PersonalData, Property, Tenancy } from '@immonext/types';
import { AlertTriangle, ArrowLeft, Eye, FileSignature, FileText, Landmark, Upload, Users, X } from 'lucide-react';
import Link from 'next/link';
import { useEffect, useMemo, useState } from 'react';
import { DataCard, Field, initials, Pill, readFileAsDataUrl } from './DocumentGeneratorParts';

export interface PersonForCertificate {
    firstName: string;
    lastName: string;
    isPrimary: boolean;
    taxId: string;
...
interface TenantCertificateModalProps {
    open: boolean;
    onClose: () => void;
    userId: string;
    property: Property;
...
function isPersonComplete(person: PersonForCertificate): boolean {
...
export function TenantCertificateModal({ open, onClose, userId, property, unitLabel, tenancy, persons }: TenantCertificateModalProps) {
    ...
    // Stable for the lifetime of this open modal instance — not a real
    // persisted counter, just a readable per-tenancy reference.
    const documentNumber = useMemo(
        () => `MB-${new Date().getFullYear()}-${String(tenancy?.tenancyId ?? 0).padStart(3, '0')}`,
        [tenancy?.tenancyId]
    );
                ...
                <div className="flex items-start justify-between gap-4 px-6 pt-6 pb-4 border-b border-border shrink-0">
                    <div className="min-w-0">
                        <h2 className="text-lg font-semibold text-foreground">
                            {view === 'preview' ? 'Vorschau · Mieterbescheinigung' : 'Mieterbescheinigung generieren'}
                        </h2>
                ...
                {view === 'review' ? (
                    <div className="px-6 py-5 overflow-y-auto flex flex-col gap-6">
                        ...
                        <div>
                            ...
                            <DataCard icon={FileSignature} title="Mietvertrag" source="Bestandsobjekt · Mietvertrag">
