...
export default function DetailCheckOverviewPage() {
  ...
  const { data: rawData, isLoading, error, deleteSelected } = useQuickChecks(true);
    ...
    <div className="min-h-screen bg-background">
      <main className="container mx-auto px-4 py-8">
        ...
        <Header
          ...
          actions={
            <Link href="/property-valuation/detail-check/property-data">
              <Button
                label={BUTTON_DETAILS.AddDetailCheck.label}
                icon={<BUTTON_DETAILS.AddDetailCheck.icon />}
                variant="primary"
                hideLabelOnMobile
              />
            </Link>
        ...
        {!authLoading && !isLoading && !error && (
          <div className="mt-4">
            {/* No data at all — no detail check has been started yet */}
            {allEntries.length === 0 && (
              <NoResult
                title="Noch keine Detailbewertungen vorhanden"
                message="Starten Sie eine Detailbewertung aus einer Ersteinschätzung oder legen Sie eine neue an."
                className="py-24"
              />
            )}
